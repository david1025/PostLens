...
class UdpMessage {
  ...
  UdpMessage(this.content, this.isSent, this.timestamp,
      {this.isSystem = false});
}

class UdpPane extends ConsumerStatefulWidget {
  const UdpPane({super.key});

  @override
  ConsumerState<UdpPane> createState() => _UdpPaneState();
}

class _UdpPaneState extends ConsumerState<UdpPane>
  ...
  final FocusNode _targetHostFocusNode = FocusNode();
  final FocusNode _targetPortFocusNode = FocusNode();
  ...
  final List<UdpMessage> _messages = [];
  ...
  final FocusNode _localPortFocusNode = FocusNode();
  String _localHost = '0.0.0.0';
  List<String> _localIps = ['0.0.0.0', '127.0.0.1'];
  ...
  void initState() {
    ...
    _targetHostFocusNode.addListener(() => setState(() {}));
    _targetPortFocusNode.addListener(() => setState(() {}));
    _localPortFocusNode.addListener(() => setState(() {}));
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final req = ref.read(requestProvider);
      if (req.url.isNotEmpty) {
        final url = req.url.replaceFirst('udp://', '');
        final parts = url.split(':');
        _targetHostController.text = parts[0];
        if (parts.length > 1) {
          _targetPortController.text = parts[1];
        }
      } else {
        _targetHostController.text = '127.0.0.1';
        _targetPortController.text = '9001';
      }
    });
  }
  ...
  void dispose() {
    ...
    _targetHostFocusNode.dispose();
    _targetPortFocusNode.dispose();
    _messageController.dispose();
    _scrollController.dispose();
    _tabController.dispose();
    _localPortController.dispose();
    _localPortFocusNode.dispose();
  ...
  void _toggleConnection() async {
    ...
    if (_isBound) {
      ...
      setState(() {
        _isBound = false;
        _messages.add(UdpMessage(
            t['disconnected'] ?? 'Unbound', false, DateTime.now(),
            isSystem: true));
      });
    } else {
      try {
        final host = _localHost;
        final port = int.tryParse(_localPortController.text.trim()) ?? 0;

        final targetHost = _targetHostController.text.trim();
        final targetPort = _targetPortController.text.trim();
        ref
            .read(requestProvider.notifier)
            .updateUrl('udp://$targetHost:$targetPort');
        ...
        setState(() {
          _isBound = true;
          _messages.add(UdpMessage(
              '${t['bound_to'] ?? 'Bound to'} ${_socket!.address.address}:${_socket!.port}',
              false,
              DateTime.now(),
              isSystem: true));
        });
              ...
              if (d != null && mounted) {
                setState(() {
                  ...
                  if (!_hideReceive) {
                    ...
                    _messages.add(UdpMessage(
                        '${d.address.address}:${d.port} -> $content',
                        false,
                        DateTime.now()));
                  }
                });
          ...
          onDone: () {
            if (mounted) {
              setState(() {
                if (_isBound) {
                  _messages.add(UdpMessage(
                      t['connection_closed'] ?? 'Socket closed',
                      false,
                      DateTime.now(),
                      isSystem: true));
                  _isBound = false;
        ...
        setState(() {
          _messages.add(UdpMessage(
              '${t['failed_to_bind'] ?? 'Failed to bind'}: $e',
              false,
              DateTime.now(),
              isSystem: true));
        });
  ...
  void _sendMessage() async {
    if (!_isBound || _socket == null) return;
    ...
    final targetHost = _targetHostController.text;
    final targetPort = int.tryParse(_targetPortController.text) ?? 9001;

    try {
      final addresses = await InternetAddress.lookup(targetHost);
      if (addresses.isNotEmpty) {
        int sent = _socket!.send(data, addresses.first, targetPort);
        if (sent > 0) {
          setState(() {
            _txPackets++;
            _txBytes += data.length;
            _messages.add(UdpMessage(text, true, DateTime.now()));
          });
          if (_autoScroll) _scrollToBottom();
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to resolve target: $e')));
      }
    }
  }
              ...
              return Row(
                children: [
                  if (request.folderPath != null &&
                    ...
                    const FaIcon(FontAwesomeIcons.broadcastTower,
      ...
      children: [
        Expanded(
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(8.0),
              border:
                  Border.all(color: Colors.transparent), // Remove outer border
            ),
            child: Row(
              children: [
                Expanded(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(
                        color: _targetHostFocusNode.hasFocus
                    ...
                    child: TextField(
                      ...
                      focusNode: _targetHostFocusNode,
                      style:
                      ...
                      onChanged: (val) {
                        ref.read(requestProvider.notifier).updateUrl(
                            'udp://$val:${_targetPortController.text}');
                      },
                    ),
                  ),
                ),
                ...
                Expanded(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(
                        color: _targetPortFocusNode.hasFocus
                            ? Theme.of(context).colorScheme.primary
                            : Theme.of(context).dividerColor,
                      ),
                      borderRadius: BorderRadius.circular(4.0),
                    ...
                    child: TextField(
                      ...
                      focusNode: _targetPortFocusNode,
                      style:
                          const TextStyle(fontFamily: 'Consolas', fontSize: 12),
                      decoration: InputDecoration(
                        prefixIcon: IntrinsicWidth(
                      ...
                      onChanged: (val) {
                        ref.read(requestProvider.notifier).updateUrl(
                            'udp://${_targetHostController.text}:$val');
                      },
                    ),
                  ),
                ),
                        ...
                        Expanded(
                          child: PopupMenuButton<String>(
                            ...
                            offset: const Offset(-12,
                                4), // Offset to align dropdown left edge with the vertical divider and add some spacing below
                            onSelected: _isBound
                                ? null
                                : (String value) {
                                    setState(() {
                ...
                Expanded(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(
                        color: _localPortFocusNode.hasFocus
                            ? Theme.of(context).colorScheme.primary
                            : Theme.of(context).dividerColor,
                      ),
                      borderRadius: BorderRadius.circular(4.0),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            onPressed: _targetHostController.text.isNotEmpty
                ? _toggleConnection
                : null,
            ...
            child: Row(
              children: [
                Text(
                  _isBound ? (t['unbind'] ?? 'Unbind') : (t['bind'] ?? 'Bind'),
                  style: const TextStyle(
                      color: Colors.white,
            ...
            children: [
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  onPressed: _isBound ? _sendMessage : null,
                  style: OutlinedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    side: BorderSide(color: Theme.of(context).dividerColor),
                  ),
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.circle,
                  size: 10, color: _isBound ? Colors.green : Colors.grey),
              const SizedBox(width: 4),
              Text(
                  _isBound
                      ? (t['bound'] ?? 'Bound')
