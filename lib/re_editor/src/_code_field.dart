...
class _CodeFieldRender extends RenderBox implements MouseTrackerAnnotation {
  ...
  bool hitTest(BoxHitTestResult result, {required Offset position}) {
    ...
    if (size.contains(position)) {
      ...
      // Also check if any child span is MouseTrackerAnnotationTextSpan
      InlineSpan? trackerSpan = span;
      if (trackerSpan == null && paragraph != null) {
        // As a fallback, try to find the span in the paragraph directly
        paragraph.span.visitChildren((child) {
          if (child is MouseTrackerAnnotationTextSpan) {
            final TextRange range = paragraph.getRangeForSpan(child);
            final List<Rect> rects = paragraph.getRangeRects(range);
            for (final rect in rects) {
              if (rect.contains(position - paragraph.offset + paintOffset)) {
                trackerSpan = child;
                return false;
              }
            }
      ...
      if (trackerSpan is MouseTrackerAnnotationTextSpan) {
        result.add(HitTestEntry(_MouseTrackerAnnotationTextSpan(
          id: paragraph!.index,
          rects: paragraph
              .getRangeRects(paragraph.getRangeForSpan(trackerSpan!))
              .map((rect) {
          ...
          span: trackerSpan as MouseTrackerAnnotationTextSpan,
        )));
      } else if (trackerSpan is HitTestTarget) {
        result.add(HitTestEntry(trackerSpan as HitTestTarget));
      }
      if (_chunkIndicators
          .where((chunk) => chunk.canExpand && chunk.region.contains(position))
          .isNotEmpty) {
        _cursor = SystemMouseCursors.click;
      } else if (trackerSpan is TextSpan &&
          (trackerSpan as TextSpan).mouseCursor != MouseCursor.defer) {
        _cursor = (trackerSpan as TextSpan).mouseCursor;
      } else {
        _cursor = SystemMouseCursors.text;
      }
      hitTarget = true;
